...
def trace(htree, eye, target):
    ...
    rays = make_rays(800, 600, target - eye, 70)
    ...
    code = '''
      ...
      for (int i = 0; i < rayNum; ++i)
      {
        ...
        if (i == (rayNum + Nrays[1]) / 2)
        {
          printf("%d %f, %f, %f\\n", i, dir.x, dir.y, dir.z);
          printf("%f, %f, %f -- %d\\n", veye.x, veye.y, veye.z, hit[i]);
        }
        reinterpret_cast<point_3i*>(voxel_pos)[i] = tracer.voxelPos;
      }
    '''
    weave.inline(code, ["bricks", "grids", "root", "eye", "rays", "voxel_pos", "hit"], headers=['"stdafx.h"', '"bittree.h"'], include_dirs=['.', '../cpp'], force=1)
